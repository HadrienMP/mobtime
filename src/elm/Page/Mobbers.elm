...
import Html exposing (Html, button, div, h2, header, i, input, li, nav, strong, text, ul)
import Html.Attributes exposing (class, id, placeholder, required, type_, value)
...
bodyView model session toMain identityMsgToMain =
    [ nav
        []
        [ button [ onClick <| toMain <| Leave ] [ i [ class "fas fa-home" ] [] ]
        , button [] [ i [ class "fas fa-clock" ] [] ]
        , button [] [ i [ class "fas fa-users" ] [] ]
        , button [] [ i [ class "fas fa-id-card" ] [] ]
        ]
    , header
        []
        [ h2
